...
init().catch((error) => {
  console.error(error);
  el.status.textContent = "Could not load exam_source.txt. Run from a local web server in this folder.";
});
...
async function init() {
  ...
  if (!state.questions.length) {
    el.status.textContent = "No questions found in source file.";
    return;
  }
...
async function loadSourceText() {
  const response = await fetch("exam_source.txt");
  if (!response.ok) {
...
function parseQuestions(raw) {
  ...
  for (const rawLine of lines) {
    ...
    if (line.startsWith("<question>")) {
      ...
      current = {
        text: normalize(line.replace("<question>", "")),
        variants: []
      };
      continue;
    }
...
function finalizeCurrentQuestion(target, current) {
  ...
  target.push({
    text: current.text,
    options: current.variants.slice(0, 5),
    correctIndex: 0
  });
}
